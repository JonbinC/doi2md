...
def build_agent_commands(project_root: Path | None = None) -> dict[str, Any]:
    ...
    commands: dict[str, Any] = {
        ...
        "parse_file": "mdtero parse --file <paper.pdf|paper.epub|paper.html|paper.xml> --trace --wait --timeout 600 --json",
        ...
        "extension_handoff_file": "mdtero parse --file <paper.pdf|paper.epub|paper.html|paper.xml> --trace --wait --timeout 600 --json",
        ...
        "rag_build": "mdtero rag build --wait --json",
...
def build_project_bridge(project_root: Path | None = None) -> dict[str, Any]:
    ...
    if state is None:
        ...
        if state.server_project_id:
            ...
            next_commands = _dedupe_commands([commands["project_status"], commands["parse_pending"], commands["refresh"], commands["rag_query"]])
        elif failed:
            status = "needs_attention"
            reason_code = "project_has_failed_items"
...
def _dashboard_setup_handoff_json_payload(commands: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "auth_boundary": {
            ...
            "dashboard_secret_retention": "The full secret is shown once and may remain only in the current page session for install prompts until refresh or explicit clear. Persistent dashboard lists show only the prefix identifier.",
        },
        "api_key": {
...
def _agent_handoff_protocol(commands: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "commands": [commands.get("rag_status"), ONE_COMMAND_RAG_BOOTSTRAP, commands.get("rag_query"), commands.get("rag_build")],
...
def serve_project_context(project_root: Path | None = None) -> None:
    try:
        ...
        raise RuntimeError(
            ...
            "reinstall the public client with `uv tool install --force git+https://github.com/JonbinC/doi2md.git`. "
