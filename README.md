...
- Discovery defaults to local multi-source search (`--sources free_core`: OpenAlex, Semantic Scholar, Crossref, arXiv, PubMed, EuropePMC, PMC, bioRxiv/medRxiv, dblp, DOAJ, Zenodo, HAL, IACR, OpenAIRE, ChemRxiv; `--sources all` adds CORE/CiteSeerX/BASE/SSRN/Google Scholar/Unpaywall/IEEE/ACM). Optional free keys improve quotas. Sci-Hub download is opt-in and disabled by default. `mdtero discover "<query>" --limit 5 --interactive` opens a paging/refinement session; `--source server` is only a Mdtero API proxy fallback.
